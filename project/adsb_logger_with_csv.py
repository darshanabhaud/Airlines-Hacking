...
with open("flight_log.csv", "w", newline="") as csvfile:
    ...
    writer.writerow(["ICAO", "Latitude", "Longitude", "Speed (kt)"])
    for icao, positions in aircraft_paths.items():
        for lat, lon, speed in positions:
            writer.writerow([icao, lat, lon, speed])
print("[✔] Flight log saved to flight_log.csv")
...
plt.title("Simulated Aircraft Flight Paths with CSV Logging")
